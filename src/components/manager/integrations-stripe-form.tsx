...
import {
  ...
  testStripeIntegrationAction,
} from "@/lib/actions/integrations";
import type { StripeIntegrationView } from "@/lib/data/integrations";
...
export function IntegrationsStripeForm({
  settings,
  labels,
}: {
  settings: StripeIntegrationView;
  labels: Labels;
}) {
    ...
    <section className="premium-card p-5">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <label className="flex flex-col gap-1.5 sm:col-span-2">
          ...
          >
            <option value="test">{labels.modeSandbox}</option>
            <option value="live">{labels.modeLive}</option>
          </select>
        </label>

        <label className="flex flex-col gap-1.5 sm:col-span-2">
          <span className="text-xs font-medium text-foreground-muted">{labels.secretKey}</span>
          <input
            type="password"
            value={secretKey}
            onChange={(e) => setSecretKey(e.target.value)}
            placeholder={settings.hasSecretKey ? "••••••••" : "sk_live_… or sk_test_…"}
            autoComplete="new-password"
            className="rounded-xl border border-border bg-surface-muted px-3 py-2.5 font-mono text-sm"
          />
          {settings.hasSecretKey ? (
            <span className="text-xs text-foreground-muted">{labels.secretKeyKeep}</span>
          ) : null}
        </label>

        <label className="flex flex-col gap-1.5 sm:col-span-2">
          <span className="text-xs font-medium text-foreground-muted">{labels.webhookSecret}</span>
          <input
            type="password"
            value={webhookSecret}
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        <Button
          ...
          onClick={() =>
            run(async () => {
              const result = await saveStripeIntegrationAction({
                secretKey,
                webhookSecret,
                mode,
        ...
        <Button
          ...
          onClick={() =>
            run(async () => {
              const result = await testStripeIntegrationAction();
              if (!result.ok) {
                setIsError(true);
                setMessage(result.error);
                router.refresh();
      ...
      {message ? (
        <p className={cn("mt-4 text-sm", isError ? "text-rose-300" : "text-emerald-300")}>
          {message}
        </p>
      ) : null}
    </section>
