...
import { getPublicAnnouncements } from "@/lib/public-api/content";
...
/** GET /api/public/announcements?locationId=|&locationSlug=&organizationSlug= */
export async function GET(request: NextRequest) {
  pruneRateLimitBuckets();
  const limit = checkRateLimit(clientKey(request, "public:announcements"), {
    limit: 60,
    windowMs: 60_000,
  });
  if (!limit.ok) {
  ...
  try {
    ...
    console.error("[public:announcements]", error);
